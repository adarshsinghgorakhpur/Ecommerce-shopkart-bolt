  ...
  endpoints: (builder) => ({
    ...
    getCoupon: builder.query<Coupon | null, string>({
      queryFn: async (code) => {
        ...
        if (data.valid_until && new Date(data.valid_until) < new Date()) return { data: null };
        if (data.usage_count >= data.usage_limit) return { data: null };
        return { data: data as Coupon };
      },
    }),
    getOrders: builder.query<Order[], string>({
      queryFn: async (userId) => {
        const { data, error } = await supabase.from('orders').select('*, items:order_items(*), address:addresses(*)').eq('user_id', userId).order('created_at', { ascending: false });
        if (error) return { error: { status: 500, data: error.message } };
    ...
    createOrder: builder.mutation<Order, { userId: string; addressId: string; items: OrderItem[]; subtotal: number; discount: number; deliveryCharge: number; total: number; paymentMethod: string; couponCode: string }>({
      queryFn: async (d) => {
        const { data: order, error } = await supabase.from('orders').insert({
          user_id: d.userId, address_id: d.addressId, subtotal: d.subtotal, discount: d.discount,
          delivery_charge: d.deliveryCharge, total: d.total, payment_method: d.paymentMethod,
          coupon_code: d.couponCode, status: 'confirmed',
        }).select().single();
        if (error) return { error: { status: 500, data: error.message } };
...
export const {
  useGetProductsQuery, useGetProductQuery, useGetCategoriesQuery, useGetBannersQuery,
  useGetReviewsQuery, useGetCouponQuery, useGetOrdersQuery, useCreateOrderMutation,
