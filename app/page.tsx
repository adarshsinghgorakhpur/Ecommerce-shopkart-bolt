...
export default function HomePage() {
  ...
  const { data: trendingProducts, isLoading: trendingLoading } = useGetProductsQuery({ trending: true, limit: 10 });
  const { data: flashSaleProducts, isLoading: flashLoading } = useGetProductsQuery({ flashSale: true, limit: 5 });
  ...
  return (
    <div className="container mx-auto px-4 py-6 space-y-8">
      ...
      <section>
        ...
        {categoriesLoading ? (
          <div className="grid grid-cols-4 gap-4">{Array.from({ length: 8 }).map((_, i) => <div key={i} className="flex flex-col items-center gap-2"><div className="h-20 w-20 rounded-full bg-muted animate-pulse" /><div className="h-4 w-16 bg-muted animate-pulse rounded" /></div>)}</div>
        ) : categories ? <CategoryGrid categories={categories} /> : null}
      </section>
      {flashLoading ? <div className="rounded-xl border p-6"><div className="h-6 w-40 bg-muted animate-pulse rounded" /><div className="grid grid-cols-5 gap-3 mt-4">{Array.from({ length: 5 }).map((_, i) => <ProductCardSkeleton key={i} />)}</div></div> : flashSaleProducts?.length ? <FlashSaleSection products={flashSaleProducts} /> : null}
      <section>{trendingLoading ? <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">{Array.from({ length: 10 }).map((_, i) => <ProductCardSkeleton key={i} />)}</div> : trendingProducts ? <TrendingSection products={trendingProducts} /> : null}</section>
