...
export interface ProductImage {
  id: string; product_id: string; url: string; alt_text: string;
  sort_order: number; is_primary: boolean;
}

export interface ProductVariant {
  id: string; product_id: string; variant_type: string; variant_value: string;
  price_adjustment: number; stock: number; sku: string; is_default: boolean;
}
...
export interface Review {
  ...
  title: string; comment: string; is_verified_purchase: boolean; created_at: string;
}

export interface User {
  id: string; full_name: string; phone: string; avatar_url: string; email?: string;
}
...
export interface Order {
  id: string; user_id: string; address_id: string; status: OrderStatus;
  subtotal: number; discount: number; delivery_charge: number; total: number;
  payment_method: string; coupon_code: string; created_at: string; updated_at: string;
  items?: OrderItem[]; address?: Address;
}

export interface OrderItem {
  id: string; order_id: string; product_id: string; product_name: string;
  variant_info: string; quantity: number; price: number; image_url: string;
}
...
export interface Banner {
  ...
  link_url: string; sort_order: number; is_active: boolean;
}

export interface Coupon {
  id: string; code: string; description: string; discount_type: 'percentage' | 'flat';
  discount_value: number; min_order_value: number; max_discount: number;
